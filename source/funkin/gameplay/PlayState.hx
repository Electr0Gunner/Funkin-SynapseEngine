...
import funkin.ui.PauseSubState;
import funkin.ui.ChartingState;
import funkin.objects.SwagCamera;
import funkin.objects.FunkinVideo;
import funkin.objects.FunkinVideo;
import funkin.ui.GitarooPause;
import funkin.ui.AnimationDebug;
import funkin.ui.GameOverSubstate;
import funkin.ui.VideoState;
import funkin.ui.StoryMenuState;
import funkin.ui.FreeplayState;
import funkin.ui.LoadingState;
import funkin.gameplay.song.Song;
import funkin.objects.DialogueBox;
import funkin.objects.NoteSplash;
import funkin.gameplay.song.Section.SwagSection;
import funkin.gameplay.song.Song.SwagSong;
...
import funkin.graphics.shader.BuildingShaders.BuildingShader;
import funkin.graphics.shader.BuildingShaders;
import funkin.graphics.shader.ColorSwap;
import funkin.options.PreferencesMenu;
import funkin.objects.Character;
import funkin.objects.Boyfriend;
import funkin.objects.Note;
import funkin.objects.HealthIcon;
import funkin.gameplay.stage.BGSprite;
import funkin.gameplay.stage.BackgroundDancer;
import funkin.gameplay.stage.BackgroundGirls;
import funkin.gameplay.stage.TankmenBG;
...
class PlayState extends funkin.ui.MusicBeatState
{
	...
	var tankBricks:BGSprite; //Sucky layering until we get the custom stages and rendering order in.
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'pico' | 'blammed' | 'philly':
				...
				var city:FlxSprite = new FlxSprite(-255, 45).loadGraphic(Paths.image('philly/city', 'week3'));
				...
				var streetBehind:FlxSprite = new FlxSprite(-299, 144).loadGraphic(Paths.image('philly/behindTrain', 'week3'));
				add(streetBehind);
				...
				var street:FlxSprite = new FlxSprite(streetBehind.x, streetBehind.y).loadGraphic(Paths.image('philly/street', 'week3'));
				add(street);
			...
			case 'guns' | 'stress' | 'ugh':
				...
				var bg:BGSprite = new BGSprite(-400, -400, 0, 0);
				bg.load('tankSky', 'week7');
				add(bg);

				var tankSky:BGSprite = new BGSprite(FlxG.random.int(-700, -100), FlxG.random.int(-20, 20), 0.1, 0.1);
				tankSky.load('tankClouds', 'week7');
				tankSky.active = true;
				tankSky.velocity.x = FlxG.random.float(5, 15);
				add(tankSky);

				var tankMountains:BGSprite = new BGSprite(-500, -32, 0.2, 0.2);
				tankMountains.load('mountains2', 'week7');
				tankMountains.setGraphicSize(Std.int(tankMountains.width * 1.2));
				tankMountains.updateHitbox();
				add(tankMountains);

				var tankBuildings:BGSprite = new BGSprite(-260, -35, 0.30, 0.30);
				tankBuildings.load('tankBuildings', 'week7');
				tankBuildings.setGraphicSize(Std.int(tankBuildings.width * 1.1));
				tankBuildings.updateHitbox();
				add(tankBuildings);

				var tankRuins:BGSprite = new BGSprite(-200, 150, 0.35, 0.35);
				tankRuins.load('cityruins2', 'week7');
				tankRuins.setGraphicSize(Std.int(tankRuins.width * 1.1));
				tankRuins.updateHitbox();
				add(tankRuins);

				var smokeLeft:BGSprite = new BGSprite(-200, -100, 0.4, 0.4);
				smokeLeft.load('smokeLeft', 'week7', ['SmokeBlurLeft'], true);
				add(smokeLeft);

				var smokeRight:BGSprite = new BGSprite(1100, -100, 0.4, 0.4);
				smokeRight.load('smokeRight', 'week7', ['SmokeRight'], true);
				add(smokeRight);

				// tankGround.

				tankWatchtower = new BGSprite(100, 50, 0.5, 0.5);
				tankWatchtower.load('tankWatchtower', 'week7', ['watchtower gradient color']);
				add(tankWatchtower);

				tankGround = new BGSprite(300, 300, 0.5, 0.5);
				tankGround.load('tankRolling', 'week7', ['BG tank w lighting'], true);
				add(tankGround);
				...
				var tankGround:BGSprite = new BGSprite(-420, -150);
				tankGround.load('tankGround', 'week7');
				tankGround.setGraphicSize(Std.int(tankGround.width * 1.15));
				tankGround.updateHitbox();
				add(tankGround);

				tankBricks = new BGSprite(438, 715);
				tankBricks.load('bricksGround', 'week7');

				moveTank();

				// smokeLeft.screenCenter();

				var fgTank0:BGSprite = new BGSprite(-500, 650, 1.7, 1.5);
				fgTank0.load('tank0', 'week7', ['fg']);
				foregroundSprites.add(fgTank0);

				var fgTank1:BGSprite = new BGSprite(-300, 750, 2, 0.2);
				fgTank1.load('tank1', 'week7' , ['fg']);
				foregroundSprites.add(fgTank1);

				// just called 'foreground' just cuz small inconsistency no bbiggei
				var fgTank2:BGSprite = new BGSprite(450, 940, 1.5, 1.5);
				fgTank2.load('tank2', 'week7' , ['foreground']);
				foregroundSprites.add(fgTank2);

				var fgTank4:BGSprite = new BGSprite(1300, 900, 1.5, 1.5);
				fgTank4.load('tank4', 'week7' , ['fg']);
				foregroundSprites.add(fgTank4);

				var fgTank5:BGSprite = new BGSprite(1620, 700, 1.5, 1.5);
				fgTank5.load('tank5', 'week7' , ['fg']);
				foregroundSprites.add(fgTank5);

				var fgTank3:BGSprite = new BGSprite( 1300, 1200, 3.5, 2.5);
				fgTank3.load('tank3', 'week7' , ['fg']);
	...
	override public function update(elapsed:Float)
	{
		...
		if (controls.PAUSE && startedCountdown && canPause)
		{
			...
			if (FlxG.random.bool(0.1))
			{
				...
				FlxG.switchState(GitarooPause.new);
			}
			else
			{
