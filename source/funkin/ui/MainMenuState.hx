...
import funkin.options.AtlasMenuList;
import funkin.options.MenuList;
import funkin.options.OptionsState;
import funkin.options.PreferencesMenu;
import funkin.options.Prompt;
...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		FlxG.cameras.reset(new funkin.objects.SwagCamera());
		FlxG.camera.follow(camFollow, null, 0.06);
		// FlxG.camera.setScrollBounds(bg.x, bg.x + bg.width, bg.y, bg.y + bg.height * 1.2);

		var versionShit:FlxText = new FlxText(5, FlxG.height - 18, 0, "v" + Application.current.meta.get('version') + ' (Synapse v${FunkinGame.ENGINE_VERSION})', 12);
